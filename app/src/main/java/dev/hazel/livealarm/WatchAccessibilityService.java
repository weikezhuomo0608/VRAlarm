...
public class WatchAccessibilityService extends AccessibilityService {
    ...
    private void recover(){Prefs p=new Prefs(this);long now=SystemClock.elapsedRealtime();if(p.enabled()&&p.watchingNow()&&!GuardianService.running&&(lastAttempt==0||now-lastAttempt>=300000)){lastAttempt=now;if(GuardianService.send(this,"CHECK"))p.log("system","守候辅助请求恢复服务","已向系统请求恢复中断的守候服务");}}
