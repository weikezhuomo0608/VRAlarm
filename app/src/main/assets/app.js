...
let seq = 0, awaiting = /* @__PURE__ */ new Map(), native = typeof window.HazelNative !== "undefined", route = "home", S = null, configFingerprint = "", busy = false, ruleDraft = null, ruleTarget = "", alarmPainted = false, zoneList = [];
let compatibilityAction = null, compatibilitySaving = false, anchorDraft = null;
const WATCH_STALL_MS = 10 * 6e4;
...
function api(action, data = {}, timeoutMs = 1e4) {
  if (!native) {
    ...
    if (action === "save") {
      ...
      if (data.silentMinutes !== void 0) {
        const minutes = data.silentMinutes;
        previewState.config.silentUntil = minutes === 0 ? 0 : minutes < 0 ? Number.MAX_SAFE_INTEGER : Date.now() + minutes * 6e4;
        delete previewState.config.silentMinutes;
      }
      return Promise.resolve(previewState.config);
    }
    if (action === "resolveAnchor") return Promise.resolve({ id: data.id || "preview", room: Number(data.room) || 1713546334, uid: 1298779265, uidText: "1298779265", name: "灰泽满 Hazel", avatar: false });
...
function clock(ms) {
  if (!ms) return "";
  try {
    return new Intl.DateTimeFormat("zh-CN", { timeZone: (S == null ? void 0 : S.zone) || void 0, hour: "2-digit", minute: "2-digit", hour12: false }).format(new Date(ms));
  } catch (e) {
...
const QUIET_FOREVER_MS = 5 * 365 * 24 * 36e5;
const quietChoiceNames = { 30: "30 分钟", 60: "1 小时", 120: "2 小时" };
function pollLabel(seconds) {
  const n = Number(seconds) || 0;
  return n >= 60 ? n % 60 ? `${Math.floor(n / 60)} 分 ${n % 60} 秒` : `${n / 60} 分钟` : `${n} 秒`;
}
...
function quietLeftLabel(nowMs) {
  const q = quietState(nowMs);
  if (!q.on) return "";
  if (q.forever) return "直到你手动恢复";
  const minutes = Math.max(1, Math.ceil(q.left / 6e4));
...
function seedSwatch(hex, config) {
  ...
  return night ? cssRgb(mixRgb(seed, [255, 255, 255], 0.5)) : cssRgb(seed);
}
...
function bindSystemTheme() {
  ...
  const handler = () => {
    if (S && S.config && S.config.theme === "system") applyTheme();
  };
...
function applyTheme() {
  ...
  if (seed) {
    ...
    setVar("--on-primary", lumaRgb(primary) > 0.179 ? "#111318" : "#ffffff");
  } else {
    setVar("--primary", null);
    setVar("--on-primary", null);
  }
  ...
  if (layer) {
    if (S.backgroundSet) {
      const stamp2 = S.backgroundRevision || S.backgroundSet;
      layer.style.backgroundImage = `url(/background/current?v=${stamp2})`;
...
function watchNoticeContent() {
  var _a;
  const status = ((_a = S.watchNotification) == null ? void 0 : _a.status) || (S.running ? "unknown" : S.enabled ? S.schedulePaused ? "paused" : "interrupted" : "stopped");
  const copy = {
...
function home() {
  ...
  const paused = !!(S.enabled && (S.schedulePaused === void 0 ? !S.inside : S.schedulePaused));
  ...
  if (S.enabled && !S.running && !paused) html += `<div class="card warning"><p>守候服务已中断${beat ? "，上次心跳在约 " + Math.round(silent / 6e4) + " 分钟前" : "，尚未收到服务心跳"}，期间不会检测开播。应用每 15 分钟会尝试自动恢复；若经常中断，请在系统设置中允许“自启动”，把电池策略设为“不限制”，并在最近任务里长按本应用的卡片将它<strong>锁定</strong>——从最近任务划掉应用时，系统会连恢复用的闹钟一起停掉。</p><button class="text-button" data-action="start">立即重新开启 ${icon("arrow")}</button>${p.accessibility ? "" : `<button class="text-button" data-action="accessibilityInfo">开启无障碍守候辅助 ${icon("arrow")}</button>`}</div>`;
  else if (S.enabled && !paused && stalled) html += `<div class="card"><p class="sub">守候服务仍在运行，只是最近一次心跳在约 ${Math.round(silent / 6e4)} 分钟前——手机休眠或省电策略会同时推迟心跳与检测，恢复亮屏后会自动接上。</p></div>`;
  if (paused) html += S.hasWindow === false ? `<div class="card warning"><p>当前是自定义时段，但<strong>没有任何启用中的时段</strong>：守候会一直暂停，不会检测开播，也不会响铃。切到「全天提醒」，或添加一个时段即可恢复。</p><button class="text-button" data-route="schedule">去设置时段 ${icon("arrow")}</button></div>` : `<div class="card"><div class="row"><div class="label-icon">${icon("moon")}</div><div class="grow"><h3>时段外 · 守候已暂停</h3><p class="sub">${S.nextBoundary ? clock(S.nextBoundary) + " 进入时段时自动恢复；" : ""}期间不联网、不检测开播。</p></div><button class="text-button" data-route="schedule">时段设置 ${icon("arrow")}</button></div></div>`;
...
function editAnchor(id) {
  ...
  anchorDraft = existing ? { id: existing.id, name: existing.name, uid: readUid(existing), room: existing.room, resolvedRoom: existing.room, enabled: existing.enabled, alarm: existing.alarm !== false, avatar: !!existing.avatar } : { id: "", name: "", uid: "", room: "", resolvedRoom: 0, enabled: true, alarm: true, avatar: false };
  const d = anchorDraft;
...
function uidDigitsOnly(value) {
  const t = uidText(value);
  return /^\d+$/.test(t) && t !== "0" ? t : "";
}
function readUid(source) {
  if (!source) return "";
  const text = uidText(source.uidText);
  return text || uidText(source.uid);
}
async function resolveAnchorDraft() {
  ...
  if (!parsed) {
    if (error) error.textContent = "请填写主播 UID（如 3546729368520811）、主页链接，或直播间链接（live.bilibili.com/…）";
    return;
  }
  ...
  try {
    const found = await api("resolveAnchor", { room, uid: parsed.uid || "", id: anchorDraft.id }, 6e4);
...
async function toggleAnchor(id) {
  ...
  try {
    stateRevision++;
    S.anchors = await api("saveAnchor", { anchor: { id: a.id, name: a.name, uid: readUid(a), room: a.room, enabled: !a.enabled, alarm: a.alarm !== false } });
    render();
    await window.refreshNative(true);
  } catch (e) {
    toast(e.message);
...
async function toggleRing(id) {
  ...
  try {
    stateRevision++;
    S.anchors = await api("saveAnchor", { anchor: { id: a.id, name: a.name, uid: readUid(a), room: a.room, enabled: a.enabled, alarm: !a.alarm } });
    render();
    await window.refreshNative(true);
  } catch (e) {
    toast(e.message);
...
function scheduleImageSrc(id, revision) {
  return "/schedule/" + id + ".img?v=" + encodeURIComponent(revision || "0");
}
...
async function refreshSchedulePicture(id) {
  ...
  if (!scheduleDraft || scheduleDraft.id !== id) return;
  scheduleDraft.hasImage = !!info.hasImage;
  scheduleDraft.revision = info.imageRevision || "";
  paintScheduleModal();
}
...
function airingToday(rows, today, nowMinutes) {
  ...
  for (const p of rows) {
    ...
    if (begun[p.a.id] === void 0 || e.start > begun[p.a.id]) {
      begun[p.a.id] = e.start;
      airing[p.a.id] = e.id;
    }
  }
  return airing;
...
function ruleListKey(target) {
  return target === "high" ? "highWindows" : "windows";
}
const RULE_PRESETS = {
  night: ["凌晨守候", 60, 360],
  overnight: ["深夜守候", 1380, 420],
  evening: ["晚间守候", 1140, 1380],
  highMorning: ["上午高频", 480, 720],
  highAfternoon: ["下午高频", 840, 960],
  highEvening: ["晚间高频", 1200, 0]
};
function editRule(id, preset, target) {
  ruleTarget = target === "high" ? "high" : "";
  const high = ruleTarget === "high";
  const existing = (S.config[ruleListKey(ruleTarget)] || []).find((w2) => w2.id === id);
  ruleDraft = existing ? clone(existing) : high ? { id: "h" + Date.now(), name: "高频时段", start: 480, end: 720, days: 127, enabled: true } : { id: "r" + Date.now(), name: "自定义时段", start: 60, end: 360, days: 127, enabled: true };
  if (preset && RULE_PRESETS[preset]) {
    const [name, start, end] = RULE_PRESETS[preset];
    Object.assign(ruleDraft, { name, start, end });
  }
  const w = ruleDraft;
  const hint = high ? "结束早于开始，自动跨到次日（例如 20:00–00:00 就是晚间）。这些时段内按「检测间隔」检测，其余时间放慢到 2 分钟省电；不影响提醒时段与响铃。" : "结束早于开始，自动跨到次日。开始与结束相同，表示从该时刻起持续 24 小时。保存后会切换为自定义时段模式。";
...
async function saveRule() {
  ...
  try {
    ...
    const rules = (S.config[ruleListKey(ruleTarget)] || []).filter((x) => x.id !== w.id);
    rules.push(w);
...
function testDialog() {
  const c = S.config, quiet = quietState().on;
...
function livePicker() {
  ...
  const row = (a) => {
    const s = a.snapshot || {};
    return `<button class="live-picker-row" data-anchor-open="${esc(a.id)}">${anchorArt(a)}<span class="grow"><strong>${esc(a.name)}</strong><small>${esc(s.status === 1 ? s.title || "正在直播" : "暂未开播")}</small></span><span class="chevron">›</span></button>`;
  };
...
function toggleLivePicker() {
  if (!enabledAnchors().length) {
    livePickerOpen = false;
    toast("还没有启用中的主播");
    render();
    return;
  }
  livePickerOpen = !livePickerOpen;
  render();
  if (livePickerOpen) {
    const panel = $("#live-picker");
...
async function perform(action, anchorId = "") {
  ...
  if (action === "openLive") {
    if (anchorId || document.body.classList.contains("alarm-page")) {
      await api("openLive", { id: anchorId });
      return;
    }
    toggleLivePicker();
    return;
  ...
  if (action === "addRule") {
    editRule(null, "", anchorId === "high" ? "high" : "");
    return;
  }
  if (action === "saveRule") {
    await saveRule();
...
document.addEventListener("click", async (e) => {
  ...
  try {
    ...
    if (b.dataset.quietMinutes !== void 0) {
      await setQuiet(Number(b.dataset.quietMinutes));
      return;
    }
    if (b.dataset.action) {
      await perform(b.dataset.action, b.dataset.anchor || b.dataset.ruleTarget || "");
      return;
    }
    ...
    if (b.dataset.pet) {
      await save({ petCharacter: b.dataset.pet });
      return;
    }
    ...
    if (b.dataset.editRule) {
      editRule(b.dataset.editRule, "", b.dataset.ruleTarget);
      return;
    }
    if (b.dataset.preset) {
      editRule(null, b.dataset.preset, b.dataset.ruleTarget);
      return;
    }
    if (b.dataset.ruleToggle) {
      ...
      r.enabled = !r.enabled;
      await save(high ? { highWindows: rules } : { windows: rules });
      return;
    }
...
const PET_SPEED = 30, PET_TURN_MS = 430, PET_HOP_MIN_MS = 9e3, PET_HOP_SPAN_MS = 7e3;
const PET_LEAN_DEG = 3.4, PET_WOBBLE_DEG = 1.7, PET_BOB_PX = 1.9, PET_NOD_HZ = 0.9, PET_SWAY_HZ = 0.45;
function facePet() {
  if (pet.el) pet.el.classList.toggle("flip", pet.dir > 0);
}
...
function petOn() {
  return !!(S && S.config && S.config.pet !== false);
}
function petCharacter() {
  return (S && S.config && S.config.petCharacter) === "lvdong" ? "lvdong" : "manqu";
}
function petMotionOk() {
  return !(typeof matchMedia === "function" && matchMedia("(prefers-reduced-motion:reduce)").matches);
}
function laneSize() {
  const viewport = window.innerWidth || 360;
  return { viewport, lane: Math.min(viewport, 580) };
}
...
function petTravel() {
  const { lane } = laneSize();
  return Math.max(0, lane - (pet.el.offsetWidth || 56) - 16);
}
function posePet() {
...
function startPet() {
  if (pet.raf || !petOn() || document.hidden || !petMotionOk()) return;
  pet.last = 0;
  pet.raf = requestAnimationFrame(petTick);
}
function stopPet() {
  if (pet.raf) cancelAnimationFrame(pet.raf);
  pet.raf = 0;
}
function petTick(ts) {
  pet.raf = 0;
  ...
  const dt = Math.min(120, ts - pet.last) / 1e3;
  pet.last = ts;
  if (!pet.nextHop) pet.nextHop = ts + PET_HOP_MIN_MS + Math.random() * PET_HOP_SPAN_MS;
  if (ts >= pet.nextHop) {
    pet.nextHop = ts + PET_HOP_MIN_MS + Math.random() * PET_HOP_SPAN_MS;
    petHop();
  }
  if (ts >= pet.wait) {
...
function petHop() {
  ...
  clearTimeout(petHop.timer);
  petHop.timer = setTimeout(() => pet.el.classList.remove("hop"), 470);
}
function mountPet() {
  pet.el = $("#pet");
  pet.art = $("#pet-art");
  ...
  document.addEventListener("visibilitychange", () => {
    document.hidden ? stopPet() : startPet();
  });
...
function syncPet() {
  ...
  pet.appliedOn = on;
  pet.appliedCharacter = character;
  pet.el.hidden = !on;
