...
function editAnchor(id) {
  ...
  anchorDraft = existing ? { id: existing.id, name: existing.name, uid: existing.uid, room: existing.room, resolvedRoom: existing.room, enabled: existing.enabled, alarm: existing.alarm !== false, avatar: !!existing.avatar } : { id: "", name: "", uid: 0, room: "", resolvedRoom: 0, enabled: true, alarm: true, avatar: false };
  const d = anchorDraft;
...
async function resolveAnchorDraft() {
  ...
  if (!parsed) {
    if (error) error.textContent = "请填写主播 UID（如 1298779265）、主页链接，或直播间链接（live.bilibili.com/…）";
    return;
  }
  ...
  try {
    const found = await api("resolveAnchor", { room, uid: parsed.uid || 0, id: anchorDraft.id }, 6e4);
...
function schedule() {
  const c = S.config, n = c.windows.filter((x) => x.enabled).length;
  let bars = "";
