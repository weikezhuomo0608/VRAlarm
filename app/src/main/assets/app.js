...
function home() {
  ...
  if (S.enabled && !S.running) html += `<div class="card warning"><p>守候服务已中断${beat ? "，上次心跳在约 " + Math.round(silent / 6e4) + " 分钟前" : "，尚未收到服务心跳"}，期间不会检测开播。应用每 15 分钟会尝试自动恢复；若经常中断，请在系统设置中允许“自启动”，把电池策略设为“不限制”，并在最近任务里长按本应用的卡片将它<strong>锁定</strong>——从最近任务划掉应用时，系统会连恢复用的闹钟一起停掉。</p><button class="text-button" data-action="start">立即重新开启 ${icon("arrow")}</button>${p.accessibility ? "" : `<button class="text-button" data-action="accessibilityInfo">开启无障碍守候辅助 ${icon("arrow")}</button>`}</div>`;
  else if (S.enabled && stalled) html += `<div class="card"><p class="sub">守候服务仍在运行，只是最近一次心跳在约 ${Math.round(silent / 6e4)} 分钟前——手机休眠或省电策略会同时推迟心跳与检测，恢复亮屏后会自动接上。</p></div>`;
