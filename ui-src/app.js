...
function render(){if(!S)return;applyTheme();if(document.body.classList.contains('alarm-page')){renderAlarm();return;}header();navigation();if(route==='home')home();if(route==='anchors')anchorsPage();if(route==='week')week();if(route==='schedule')schedule();if(route==='sound')sound();if(route==='settings')settings();if(route==='history')history();}
...
function paintScheduleModal(){
    ...
    (scheduleDraft.hasImage?'<div class="section-label"><h2>周表图片对照</h2></div><div class="sched-img"><img src="/schedule/'+esc(scheduleDraft.id)+'.img" alt="周表图片"><button class="text-button" data-action="removeScheduleImage" style="margin-top:6px">删除图片 '+icon('close')+'</button></div>':'')+
...
document.addEventListener('click',async e=>{
    ...
    try{
        ...
        if(b.dataset.anchorOpen){await api('openLive',{id:b.dataset.anchorOpen});return;}
