...
**Android 8.0 及以上 · 当前版本 1.1.6（过渡版本）· 包名 `dev.hazel.livealarm.multi`**
...
- 本仓库当前版本：**1.1.6（过渡版本）**。它是为修复「开播漏响」隐患而先行发布的过渡版：功能与 1.1.5 一致，另含两处漏响加固；**暂不含「桌面小宠物」**，其美术素材的再分发授权确认后会另出一版带回。（1.1.0 首次公开时已并入内部迭代 1.1.0–1.4.3 的全部能力）
...
- **多主播守候**：**填入主播 UID**（主页 `space.bilibili.com/` 后面那串数字），或直接粘贴主播主页链接（space.bilibili.com/…）、直播间链接（live.bilibili.com/…）即可添加，自动识别名称与头像，最多 16 位；每位主播独立「检测」与「响铃」开关，可只关注不响铃。
- **（本版未含）桌面小宠物**：底部来回爬的小宠物（含会走路的「满区」本体与 7 款果冻）属于内部开发内容，因美术素材的再分发授权待确认，**未随本版发布**；确认后会在后续版本带回。
...
- **（本版未含）桌面小宠物的立绘**取自同一角色的桌面宠物《可爱满区2.0》：其中「满区」本体在该版本里是 Spine 骨骼，由 `tools/bake_pet_art.py` 离线烘焙成平铺 PNG。**这些素材未包含在本版 APK 中**（授权确认后随功能一起发布）。
