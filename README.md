...
- 本仓库版本：1.1.0（**试运行版**；内部迭代 1.1.0–1.4.3 的全部能力并入此版）
...
- **多主播守候**：填入直播间号码，或直接粘贴直播间链接（live.bilibili.com/…）、主播主页链接（space.bilibili.com/…）即可添加，自动识别名称与头像，最多 16 位；每位主播独立「检测」与「响铃」开关，可只关注不响铃。
...
- **本质是 AI 拼好钟，切勿盲目信任。** 本项目（含代码、文档与界面）全部由 ZCode 的免费模型 GLM-5.3-Flash 生成与迭代，处于试运行阶段，未经长期真机验证；请勿把它当作唯一或关键的提醒手段，重要场次建议自行留意或设置备用闹钟。
