...
- 本仓库当前版本：**1.1.6（过渡版本）**：功能与 1.1.5 齐平，重点是检测可靠性与提醒时效的改进（「持续高频守候」、起始基线的稳健处理、单个主播异常隔离）；**「桌面小宠物」暂未包含**，其美术素材确认授权后会随后续版本提供。（1.1.0 首次公开时已并入内部迭代 1.1.0–1.4.3 的全部能力）
...
- **桌面小宠物（后续版本提供）**：底部来回爬的小宠物（含会走路的「满区」本体与 7 款果冻）已在内部完成，其美术素材的再分发授权确认后会随后续版本提供。
...
- **桌面小宠物的立绘**取自同一角色的桌面宠物《可爱满区2.0》：其中「满区」本体在该版本里是 Spine 骨骼，由本项目在本机离线烘焙成平铺 PNG。这些素材将在授权确认后随功能提供（本版未包含）。
